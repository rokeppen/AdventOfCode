...
def read_file(file):
    return set(tuple(map(int, line.split(','))) for line in takewhile(lambda x: x.strip(), file)), \
           [(0 if line[11] == 'x' else 1, int(line.split('=')[1])) for line in file]
...
if __name__ == "__main__":
    ...
    print(fold_and_read(open("uta.txt")))
